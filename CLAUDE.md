...
- [ ] **TASK-09**: Instalar e configurar Devise + devise-jwt no account-service (User + JwtDenylist)
- [ ] **TASK-10**: Endpoints de auth: `POST /api/v1/auth/sign_up`, `POST /api/v1/auth/sign_in`, `DELETE /api/v1/auth/sign_out`
...
- [ ] **TASK-20**: Model `PaymentOrder` com state machine AASM (todos os estados e transições)
...
- [ ] **TASK-30**: Model `Ccb` + `InstallmentScheduler` (gera parcelas em batch na mesma transação)
